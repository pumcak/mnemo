...
const pair = async (handle: DatabaseHandle, body: unknown): Promise<Response> =>
...
describe('POST /pair', () => {
  ...
  it('registers the device and returns its identifier', async () => {
    const response = await pair(handle, { token, name: 'desktop', platform: 'windows' });
  ...
  it('refuses a wrong token and registers nothing', async () => {
    const response = await pair(handle, { token: 'guessed', name: 'desktop', platform: 'windows' });
  ...
  it('refuses a platform the contracts do not declare', async () => {
    const response = await pair(handle, { token, name: 'phone', platform: 'android' });
  ...
  it('gives each pairing its own device, since two browsers are two devices', async () => {
    const first = (await (
      await pair(handle, { token, name: 'chrome', platform: 'windows' })
    ).json()) as {
      deviceId: string;
    };
    const second = (await (
      await pair(handle, { token, name: 'firefox', platform: 'windows' })
    ).json()) as {
      deviceId: string;
    };
  ...
  it('lets a paired device send heartbeats, which was refused before pairing', async () => {
    const paired = (await (
      await pair(handle, { token, name: 'desktop', platform: 'windows' })
    ).json()) as {
      deviceId: string;
    };
